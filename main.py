...
class AutoSeller(ConfigLoader):
    ...
    async def _load_items(self) -> None:
        ...
        async for item, item_details, thumbnail in self.__fetch_items():
            ...
            # Skip if already ignored or creator blacklisted
            if (
            ...
            # NEW: Skip items that don't have a lowestResalePrice (not resellable)
            if item_details.get("lowestResalePrice") is None:
                self.blacklist.add(item_id)
                continue
            ...
            if item_obj is None:
                ...
                item_obj = Item(
                    ...
                    thumbnail=thumbnail,
                    auth=self.auth,
                    auto_seller=self  # Pass reference to AutoSeller
                )
