...
from .collectible import Collectible, PermissionDeniedError
...
class Item:
    ...
    def __init__(
        ...
        *,
        ...
        auto_seller=None  # new parameter
    ) -> None:
        ...
        self.auth = auth
        self.auto_seller = auto_seller  # store reference
            ...
            while True:
                try:
                    ...
                    match response.status:
                        ...
                        case 403:
                            # Fallback in case exception wasn't raised (shouldn't happen now)
                            if verbose:
                    ...
                    if verbose:
                        Display.error(f"Permission denied for item {self.name} – blacklisting.")
                    if self.auto_seller:
                        self.auto_seller.blacklist.add(self.id)
                        self.auto_seller.remove_item(self.id)
                    # Stop processing this item entirely
                    return sold_amount
