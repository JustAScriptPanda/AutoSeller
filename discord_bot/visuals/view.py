    ...
    def switch_buttons_disabling(self, disabled: Optional[bool] = None):
        for child in self.children:
            if isinstance(child, Button):
                child.disabled = (not child.disabled) if disabled is None else disabled
...
class ControlPanel(BaseView):
    ...
    def _permission_check(func):
        ...
        async def wrapper(self: ControlPanel, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    def make_embed(self) -> discord.Embed:
        ...
        embed.add_field(name="Lowest Resale Price", value=f"`{item.define_lowest_resale_price()}`", inline=True)
        embed.add_field(name="Latest Sale", value=f"`{item.define_latest_sale()}`", inline=True)
        embed.add_field(name="RAP", value=f"`{item.define_recent_average_price()}`", inline=True)
        embed.add_field(name="Price", value=f"`{item.price:,}`", inline=True)
        embed.add_field(name="Quantity", value=f"`{item.quantity:,}`", inline=True)
